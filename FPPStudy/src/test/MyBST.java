...
public class MyBST {
	...
	public void insert(Integer x) {
		if(find(x)) {//root ကရှိပြီးသားဆိုရင် မထည့်
			return;
		}
		if(root == null) {//အစ root ကို မရှိသေးရင် 
			root = new Node(x, null, null);//new node create လုပ်လိုက် . ပြီးပြီ
		}
		else {//အစ root ရှိပြီးသားဆိုရင်
			Node n = root;//root ရှိပြီးသား data နဲ့
			boolean inserted = false;
			while(!inserted) {
				if(x.compareTo(n.data)<0) {//pass လိုက်တဲ့ data နဲ့ rootရှိပြီးသား data နဲ့ တိုက်စစ် ငယ်ရင်
					if(n.left == null) {//root ရဲ့ left null ဖြစ်ရင်
						n.left = new Node(x, null, null);
						inserted = true;
					}//null ဖြစ်မှ ထည့်လို့ရမှာ
					else {
						//နောက်တစ်ဆင့်ဆင်းပြီး while loop ဆက်ပတ်ဖို့
						n = n.left;//for current root become next level left node - to go up again for while loop
					}
				}
